...
const TAB_ORDER: Tab[] = ["tasks", "notes", "projects", "writing", "meeting"];
...
function formatDueDate(due: string): {
  ...
  if (dStr === tomorrowStr)
    return { label: "Tomorrow", overdue: false, today: false };
  ...
  return (
    <motion.div
      ...
      className={`group flex items-start gap-3 px-4 py-3 rounded-xl border-l-2 transition-all duration-200 ${p.border} ${
        task.completed || task.archived ? "opacity-50" : "hover:-translate-y-px"
      }`}
      style={{
...
function TaskModal({ task, onSave, onClose }: TaskModalProps) {
  ...
  const [subTasks, setSubTasks] = useState<OrbitSubTask[]>(
    task?.sub_tasks ?? [],
  );
          ...
          >
            <div
              ...
              style={{
                width: `${progress}%`,
                background:
                  progress === 100 ? "rgba(52, 211, 153, 0.7)" : colorStyle.bg,
              }}
            />
          </div>
          <p
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            ...
            <div className="flex-1">
              ...
              <OrbitDatePicker value={deadline} onChange={setDeadline} />
            </div>
          </div>
          ...
          {WRITING_MODES.map(({ mode, label, description }) => (
            <button
              ...
              style={{
                background:
                  activeMode === mode
                    ? "rgba(124, 79, 240, 0.18)"
                    : "rgba(255,255,255,0.03)",
                border:
                  activeMode === mode
                    ? "1px solid rgba(124, 79, 240, 0.3)"
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{
                background:
                  "linear-gradient(to right, rgba(124, 79, 240, 0.8), rgba(59, 130, 246, 0.8))",
                color: "#fff",
              }}
            >
              {loading ? (
                ...
                <>
                  {WRITING_MODES.find((m) => m.mode === activeMode)?.label}{" "}
                  <ArrowRight size={14} />
                </>
              )}
            </button>
        ...
        <div className="flex-1 flex flex-col min-h-0 p-4 gap-3">
          {output ? (
            <textarea
              readOnly
              value={output}
              className="flex-1 min-h-0 w-full resize-none rounded-xl px-4 py-3 text-sm leading-relaxed focus:outline-none"
              style={{
                background: "rgba(255,255,255,0.03)",
                border: "1px solid rgba(255,255,255,0.08)",
                color: "var(--color-text-primary)",
            ...
            <div
              className="flex-1 min-h-0 rounded-xl flex flex-col items-center justify-center gap-3 text-center"
              style={{
                background: "rgba(255,255,255,0.03)",
                border: "1px solid rgba(255,255,255,0.08)",
                color: "var(--color-text-secondary)",
              ...
              {loading ? (
                <>
                  <Loader2
                    ...
                    style={{ color: "var(--color-purple-400)", opacity: 0.6 }}
                  />
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={!output}
              className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
              style={{
                background: output
                  ? "linear-gradient(to left, rgba(124, 79, 240, 0.8), rgba(59, 130, 246, 0.8))"
                  : "rgba(255,255,255,0.03)",
                border: output ? "none" : "1px solid rgba(255,255,255,0.06)",
                color: output ? "#fff" : "var(--color-text-muted)",
              }}
            >
              <ArrowLeft size={14} />
              Use as input
            ...
            <button
              onClick={handleCopy}
              disabled={!output}
              className="flex items-center gap-1.5 text-[11px] px-3 py-2.5 rounded-xl transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
              style={{
                color: copied
...
function MeetingModeView({
  ...
  if (showHistory) {
    const viewSession =
      selectedSession ?? (sessions.length > 0 ? sessions[0] : null);
    return (
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        <div
        ...
        >
          ...
          <button onClick={() => setShowHistory(false)} className="win-btn">
            <X size={14} />
          </button>
        </div>
              ...
              {viewSession ? (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    >
                      <StickyNote size={10} />
                      {viewSession.entries.length} note
                      {viewSession.entries.length !== 1 ? "s" : ""}
                    </span>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
        ...
        <p
          className="text-sm max-w-sm leading-relaxed mb-7"
          style={{ color: "var(--color-text-secondary)", opacity: 0.5 }}
        >
          Jot one note per point as the conversation unfolds. Luna will write a
          summary note and create a follow-up task when you end.
        </p>
        <div className="w-full max-w-xs space-y-3">
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") void handleStartMeeting();
            }}
...
export default function Orbit() {
  ...
  const prevTabRef = useRef<Tab>("tasks");
  ...
  const [projectModal, setProjectModal] = useState<OrbitProject | null | "new">(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (
      pendingTab &&
      (
        ["tasks", "notes", "projects", "writing", "meeting"] as string[]
      ).includes(pendingTab)
    ) {
      switchTab(pendingTab as Tab);
    }
  }, [consumePendingTab, switchTab]);
  ...
  const showNewButton =
    tab === "tasks" || tab === "notes" || tab === "projects";
  ...
  const enrichedProjects = projects.map((p) => {
    const linkedTasks = tasks.filter(
      (t) => p.taskIds.includes(t.id) && !t.archived,
    );
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          >
            <div className="flex items-center gap-1">
              {(
                ["tasks", "notes", "projects", "writing", "meeting"] as Tab[]
              ).map((t) => (
                <button
                  key={t}
                  onClick={() => switchTab(t)}
                  ...
                  style={{
                    background:
                      tab === t ? "rgba(124, 79, 240, 0.2)" : "transparent",
                    border:
                      tab === t
                        ? "1px solid rgba(124, 79, 240, 0.3)"
                        : "1px solid transparent",
          ...
          <div className="flex-1 relative overflow-hidden">
            <AnimatePresence mode="sync" custom={tabDirection} initial={false}>
              <motion.div
                key={tab}
                custom={tabDirection}
                    ...
                    <div className="flex items-center justify-between px-5 py-2.5 shrink-0">
                      ...
                      <div ref={sortRef} className="relative">
                        ...
                        >
                          ...
                          <ChevronDown
                            size={10}
                            className={showSortMenu ? "rotate-180" : ""}
                        ...
                        {showSortMenu && (
                          ...
                          >
                            {(["recent", "priority", "due"] as const).map(
                              (s) => (
                                <button
                                  key={s}
                                  onClick={() => {
